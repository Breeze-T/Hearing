...
public class ExcelOpen {
	...
	/**
	 * 返回指定单元格字体颜色(测试仅对HSSFWorkbook对象的excel有效,XSSFWorkbook返回值全为0)
	 * @param sheetIndex 
	 * @param rowIndex
	 * @param cellIndex
	 * @return
	 */
	public int getFontColor(int sheetIndex,int rowIndex,int cellIndex){
		Sheet sheet= wb.getSheetAt(sheetIndex);
		Row row = sheet.getRow(rowIndex);
		Cell cell = row.getCell(cellIndex);
		return wb.getFontAt(cell.getCellStyle().getFontIndex()).getColor();
	}
	...
	public Object[] readExcelObjectLine(int sheetNum, int lineNum) {
		...
		try {
			...
			int cellCount = row.getLastCellNum();
			strExcelLine = new Object[cellCount + 1];
			for (int i = 0; i <= cellCount; i++) {
		...
		try {
			...
			if (null != row.getCell(cellNum)) { // add this condition
				...
				switch (row.getCell(cellNum).getCellType()) {
					case Cell.CELL_TYPE_FORMULA : strExcelCell = "FORMULA "; break;
					case Cell.CELL_TYPE_STRING : strExcelCell = row.getCell(cellNum).getStringCellValue(); break;
					case Cell.CELL_TYPE_NUMERIC : strExcelCell = formatter.format(row.getCell(cellNum).getNumericCellValue()); break;
					case Cell.CELL_TYPE_BLANK : strExcelCell = ""; break;
		...
		try {
			...
			if (null != row.getCell(cellNum)) { // add this condition
				...
				switch (row.getCell(cellNum).getCellType()) {
					case Cell.CELL_TYPE_FORMULA : strExcelCell = "FORMULA "; break;
					case Cell.CELL_TYPE_STRING : strExcelCell = row.getCell(cellNum).getStringCellValue(); break;
					case Cell.CELL_TYPE_NUMERIC : strExcelCell = row.getCell(cellNum).getNumericCellValue(); break;
					case Cell.CELL_TYPE_BLANK : strExcelCell = ""; break;
					default: strExcelCell = ""; break;
				}
			}
	...
	public static void main(String args[]) {
		System.out.println("----------nain---1----");
		DecimalFormat formatter = new DecimalFormat("#.##");
		//File file = new File("C:\\Users\\fanHua\\Desktop\\新建 Microsoft Excel 工作表.xls");
		File file = new File("E:\\work201612\\smartCampus\\src\\main\\webapp\\resources\\excel\\teacherModel.xlsx");
		...
		System.out.println("----------nain--2-----");
		for (int i = 1; i <= count; i++) {
			System.out.println("----------nain--for-----");
